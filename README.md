...
| **浏览仓库文件**        | 工具条“文件”按需展开远端工作区的目录树，预览带行号与语法高亮的文本，或 PNG、JPEG、GIF、WebP 图片；二进制、过大、编码不支持、无权限的文件及符号链接、子模块会给出说明。浏览只读，不会修改远端文件。 |
...
| **PR / MR 协作状态**    | 工具条“PR/MR”读取所选远端收到的 GitHub PR 或 GitLab MR，可切换远端、筛选开放中／全部、分页和刷新；点击标题在浏览器查看详情。私有仓库使用当前视图中的临时访问令牌，支持 HTTPS 自建 GitLab。 |
